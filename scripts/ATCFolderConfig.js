...
export class ATCFolderConfig extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static PARTS = {
    body: {
      template: `modules/${MODULE_ID}/templates/cover-effects-menu-app-folder-config.html`,
    },
    footer: {
  ...
  async _prepareContext(options) {
    ...
    const folders = CoverEffect.folders;
    if ( !folders.has(this.folderId) ) await CoverEffect.addFolder({ id: this.folderId });
    const folder = CoverEffect.folders.get(this.folderId);
  ...
  async #processSubmitData(_event, _form, submitData, _options) {
    ...
    return CONFIG[MODULE_ID].CoverEffect.addFolder(submitData.folder);
  }
